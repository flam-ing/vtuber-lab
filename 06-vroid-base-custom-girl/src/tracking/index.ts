...
export interface MingoTrackingDebug {
  ...
  /** 얼굴 468점 (정규화 0..1, y↓) */
  face: LandmarkPt[] | null
  /** 손 0~2개 × 21점 */
  hands: LandmarkPt[][]
  /** 전신 포즈 33점 (이미지 좌표) */
  pose: LandmarkPt[] | null
  faceSeen: boolean
  handCount: number
  tracked: number
  camW: number
  ...
  const processFrame = (nowMs: number): void => {
    ...
    if (!absent || frameIdx % ABSENCE_FACE_EVERY_N === 0) try {
      ...
      if (mat && bs && bs.categories.length > 0) {
        ...
        const flm = res.faceLandmarks[0]
        if (flm) dbg.face = flm.map((p) => ({ x: p.x, y: p.y }))
      } else {
        dbg.face = null
      }
    } catch (err) {
    ...
    if (!faceSeen) {
      copyFaceFields(prev, back)
      if (nowMs - lastFaceSeenMs > HAND_SEEN_TIMEOUT_MS) dbg.face = null
    }
      ...
      try {
        ...
        if (world && world.length >= 33) {
          if (img && img.length >= 33) {
            lastPoseImg = img
            lastPoseImgMs = nowMs
            dbg.pose = img.map((p) => ({ x: p.x, y: p.y }))
          }
    ...
    if (handLm && !absent && frameIdx % 2 === 1) {
      ...
      try {
        ...
        for (let i = 0; i < res.landmarks.length; i++) {
          ...
          handDebug.push(img.map((p) => ({ x: p.x, y: p.y })))
        ...
        dbg.hands = handDebug
        dbg.handCount = handDebug.length
      } catch (err) {
        logDetectErr('hand', err)
      }
    }
    if (
      nowMs - armStates.L.lastHandSeenMs > HAND_SEEN_TIMEOUT_MS &&
      nowMs - armStates.R.lastHandSeenMs > HAND_SEEN_TIMEOUT_MS
    ) {
      dbg.hands = []
      dbg.handCount = 0
    }
    if (nowMs - lastPoseImgMs > POSE_IMG_FRESH_MS) dbg.pose = null
    ...
    dbg.faceSeen = faceSeen
    dbg.tracked = back.tracked
    dbg.camW = videoEl.videoWidth
    dbg.camH = videoEl.videoHeight
  }
